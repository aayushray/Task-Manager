package com.example.task.controllers;

import com.example.task.domain.dto.TaskListDto;
import com.example.task.domain.entities.TaskList;
import com.example.task.mappers.TaskListMapper;
import com.example.task.services.TaskListService;
import org.springframework.web.bind.annotation.*;

import java.util.List;
import java.util.Optional;
import java.util.UUID;

@RestController
@RequestMapping(path = "/task-lists")
public class TaskListController {

    private final TaskListService taskListService;
    private final TaskListMapper taskListMapper;

    public TaskListController(TaskListService taskListService, TaskListMapper taskListMapper) {
        this.taskListService = taskListService;
        this.taskListMapper = taskListMapper;
    }

    @GetMapping
    public List<TaskListDto> listTaskLists(){
        /*  1. Fetch the TaskLists from the dB
            2. Convert the TaskLists from entity format to DTO, and return them.
        */
        List<TaskList> fetchedServiceData = taskListService.listTaskList();
        return fetchedServiceData.
                stream().
                map(taskListMapper::toDto).
                toList();

    }

    @PostMapping
    public TaskListDto createTaskList(@RequestBody TaskListDto taskListDto){
        TaskList taskList = taskListMapper.fromDto(taskListDto);
        TaskList createdTaskList = taskListService.createTaskList(taskList);

        return taskListMapper.toDto(createdTaskList);
    }

    @GetMapping(path = "/{task_list_id}")
    public Optional<TaskListDto> getTaskList(@PathVariable("task_list_id") UUID taskListId){
        return taskListService.getTaskList(taskListId).map(taskListMapper::toDto);
    }

    @PutMapping(path = "/{task_list_id}")
    public TaskListDto updateTaskList(
            @PathVariable("task_list_id")UUID taskListId,
            @RequestBody TaskListDto taskListDto
    ){
        TaskList updatedTaskList = taskListService.updateTaskList(taskListId, taskListMapper.fromDto(taskListDto));

        return taskListMapper.toDto(updatedTaskList);
    }

    @DeleteMapping(path = "/{task_list_id}")
    public void deleteTaskList(@PathVariable("task_list_id") UUID taskListID){
        taskListService.deleteTaskList(taskListID);
    }
}

/*
1. Declare the dependencies RestController, Mapping.
2. Need to return it in the form of TaskListDTO.
3. Fetch the data from dB and convert it into DTO.
*/
